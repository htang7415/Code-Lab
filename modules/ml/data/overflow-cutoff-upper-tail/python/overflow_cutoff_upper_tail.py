...
def overflow_cutoff_upper_tail(
    ...
    if len(qualifying_overflow) == 1:
        return float(qualifying_overflow[0])

    position = (len(qualifying_overflow) - 1) * quantile
    lower_index = math.floor(position)
    upper_index = math.ceil(position)
    if lower_index == upper_index:
        return float(qualifying_overflow[lower_index])

    weight = position - lower_index
    return qualifying_overflow[lower_index] * (1.0 - weight) + qualifying_overflow[upper_index] * weight
